...
class TestCacheKey(unittest.TestCase):
    ...
    def test_key_config(self):
        ...
        assert CK.redis_hit_key == "retools:hits:2004-02-02:home:my_func:1 2 3"
...
class TestCacheRegion(unittest.TestCase):
    ...
    def test_add_region(self):
        ...
        assert CR.regions["short_term"]["expires"] == 60
    ...
    def test_generate_value(self):
        ...
        with patch("retools.global_connection._redis", mock_redis):
            ...
            exec_calls = [
                x for x in mock_pipeline.method_calls if x[0] == "execute"
            ]
            assert len(mock_pipeline.method_calls) == 11
    ...
    def test_existing_value_no_regen(self):
        ...
        results = [
            ...
            (
                {
                    b"created": "111",
                    b"value": b"S'This is a value: 1311702429.28'\n.",
                },
                "0",
            ),
        ]
        ...
        with patch("retools.global_connection._redis", mock_redis):
            ...
            assert len(mock_pipeline.method_calls) == 4
    ...
    def test_generate_value_no_stats(self):
        ...
        with patch("retools.global_connection._redis", mock_redis):
            ...
            assert len(mock_pipeline.method_calls) == 6
            assert len(exec_calls) == 1
    ...
    def test_generate_value_other_creator(self):
        ...
        mock_redis.hgetall.return_value = {
            b"created": now,
            b"value": pickle.dumps("This is a NEW value"),
        }
        with patch("retools.global_connection._redis", mock_redis):
            CR = self._makeOne()
            CR.add_region("short_term", 60)
    ...
    def test_existing_value(self):
        ...
        mock_pipeline.execute.return_value = (
            {b"created": now, b"value": pickle.dumps("This is a value")},
            "0",
        )
        with patch("retools.global_connection._redis", mock_redis):
            CR = self._makeOne()
    ...
    def test_existing_expired_value_with_lock_timeout(self):
        ...
        results = [
            ...
            (
                {
                    b"created": now - 200,
                    b"value": pickle.dumps("This is a value"),
                },
                "0",
            ),
        ]
    ...
    def test_no_value_with_lock_timeout(self):
        ...
        redis_patch = patch("retools.global_connection._redis", mock_redis)
        sleep_patch = patch("time.sleep", mock_sleep)
        with redis_patch, sleep_patch:
            CR = self._makeOne()
            CR.add_region("short_term", 60)
    ...
    def test_new_value_and_misses(self):
        ...
        with patch("retools.global_connection._redis", mock_redis):
            ...
            last_incr_call = [
                x for x in mock_pipeline.method_calls if x[0] == "incr"
            ][-1]
            assert last_incr_call[2] == {"amount": 30}
...
class TestInvalidateRegion(unittest.TestCase):
    ...
    def test_invalidate_region_empty(self):
        ...
        with patch("retools.global_connection._redis", mock_redis):
            ...
            assert len(mock_redis.method_calls) == 1

    def test_invalidate_small_region(self):
        mock_redis = Mock(spec=redis.client.Redis)
        results = [{b"keyspace"}, {b"a_func"}]
        ...
        with patch("retools.global_connection._redis", mock_redis):
            ...
            assert calls[0][1] == ("retools:short_term:namespaces",)
            assert len(calls) == 6
...
class TestInvalidFunction(unittest.TestCase):
    ...
    def test_invalidate_function_without_args(self):
        ...
        with patch("retools.global_connection._redis", mock_redis):
            ...
            calls = mock_redis.method_calls
            assert calls[0][1] == ("retools:short_term:retools:a_key:keys",)
            assert len(calls) == 2
    ...
    def test_invalidate_function_with_args(self):
        ...
        with patch("retools.global_connection._redis", mock_redis):
            ...
            assert (
                calls[0][1][0]
                == "retools:short_term:retools:a_key decarg:fred"
            )
            ...
            calls = mock_redis.method_calls
            assert (
                calls[0][1][0] == "retools:short_term:retools:a_key"
                " decarg:\u03b5\u03bb\u03bb\u03b7\u03bd\u03b9\u03ba\u03ac"
            )
            assert calls[0][0] == "hset"
            assert len(calls) == 1
